...
<thead>
  ...
  <tbody>
    ...
    <td height="25" align="left" bgcolor="">{{ $officeSubDivision->office_division_id }} / {{ $officeSubDivision->office_circle_id }}&nbsp;</td>
    <td height="25" align="left" bgcolor="">{{ $officeSubDivision->office_zone_id }}&nbsp;</td>
    <td align="left" class="action text-center">
    	{{Form::open(array('url'=>route('officesubdivision.destroy', array($officeSubDivision->id)),'method'=>'delete'))}}
			...
			<button type="submit" onclick="return confirm ('<?php echo _('Are you sure') ?>');" name="id" class="btn btn-xs btn-danger tooltip-top" title="Remove Catalog" value="{{$officeSubDivision->id}}"><i class="glyphicon glyphicon-trash"></i></button>
		{{Form::close()}}
    </td>
    </tr>
    <?php $slno++; ?>
...
<div class="col-md-4">
	<div class="panel panel-default">
        ...
        {{ Form::model($officeSubDivisionById, array('url'=>route('officesubdivision.update', $officeSubDivisionById->id),'method'=>'put','class'=>'form-horizontal')) }}
                        ...
                        @foreach($officeZoneAll as $id => $officeZone)
                            ...
                            <?php
                                ...
                                foreach ($officeCircle as $id => $name) {
                                    if($id == $officeSubDivisionById->office_circle_id){
                                        $selected = "selected";
                                    } else { $selected=''; }
            ...
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    <select name="office_division_id" class="form-control input-sm" required id="office_division_id">
                        <option></option>
                        <?php
                            $officeDivision = OfficeDivision::where('id','=',$officeSubDivisionById->office_division_id)->orderBy('name','asc')->lists('name','id'); 
                            foreach ($officeDivision as $id => $name) {
                                if($id == $officeSubDivisionById->office_division_id){
                                    $selected = "selected";
                                } else { $selected=''; }
                                echo "<option value='$id' $selected>$name</option>";
                            }
