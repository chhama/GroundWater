...
<thead>
  ...
  <tbody>
    ...
    <td height="25" align="left" bgcolor="">{{ $officeSection->office_sub_division_id }} / {{ $officeSection->office_division_id }}&nbsp;</td>
    <td height="25" align="left" bgcolor="">{{ $officeSection->office_circle_id }} / {{ $officeSection->office_zone_id }}&nbsp;</td>
    <td align="left" class="action text-center">
    	{{Form::open(array('url'=>route('officesection.destroy', array($officeSection->id)),'method'=>'delete'))}}
			...
			<button type="submit" onclick="return confirm ('<?php echo _('Are you sure') ?>');" name="id" class="btn btn-xs btn-danger tooltip-top" title="Remove Catalog" value="{{$officeSection->id}}"><i class="glyphicon glyphicon-trash"></i></button>
		{{Form::close()}}
    </td>
    </tr>
    <?php $slno++; ?>
...
<div class="col-md-4">
	<div class="panel panel-default">
        ...
        {{ Form::model($officeSectionById, array('url'=>route('officesection.update', $officeSectionById->id),'method'=>'put','class'=>'form-horizontal')) }}
                        ...
                        @foreach($officeZoneAll as $id => $officeZone)
                            ...
                            <?php
                                ...
                                foreach ($officeCircle as $id => $name) {
                                    if($id == $officeSectionById->office_circle_id){
                                        $selected = "selected";
                                    } else { $selected=''; }
